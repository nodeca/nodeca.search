...
Server.prototype.reindex_start = Promise.coroutine(function* (config) {
  ...
  let daemon_path = this.__get_daemon_path__(false);

  // remove old shadow daemon directory
  yield rimraf(daemon_path);
  ...
  this.shadow = new Daemon(daemon_path);
  yield this.shadow.start();
...
Server.prototype.reindex_done = Promise.coroutine(function* () {
  ...
  // restart sphinx after reindex is completed to flush binlog
  yield this.shadow.stop();
  yield this.shadow.start();

  let old_active = this.active;
  let old_path = this.__get_daemon_path__(true);
...
Server.prototype.reindex_abort = Promise.coroutine(function* () {
  ...
  yield rimraf(this.__get_daemon_path__(false));
...
//
Server.prototype.execute = Promise.coroutine(function* (queries) {
  let first_query = Array.isArray(queries) ? queries[0] : queries;

  if (first_query.match(/^(SELECT|SHOW)\s/i)) {
    return this.active.execute(queries);
  }
  ...
  let results = yield Promise.map(daemons.filter(Boolean), (daemon, idx) => {
    let promise = daemon.execute(queries);
...
//
Server.prototype.execute_shadow = function (queries) {
  return this.shadow.execute(queries);
};
