...
function enqueue_wrap(fn) {
  return function () {
    ...
    const call_fn = () =>
      fn.apply(this, arguments).finally(() => {
...
Daemon.prototype.__start__ = Promise.coroutine(function* start() {
  if (this.state === STATE_RUNNING) return;

  if (this.state === STATE_INIT) {
    try {
      this.config = JSON.parse(yield readFile(path.join(this.root, 'config.json'), 'utf8'));
    } catch (err) {
  ...
  try {
    ...
    //
    let result = yield stat(path.join(this.root, 'pid_file'));
...
Daemon.prototype.__create__ = Promise.coroutine(function* create(config) {
  yield mkdirp(this.root);
  yield write(path.join(this.root, 'config.json'), JSON.stringify(config, null, 2) + '\n');
  yield this.__start__();
});


Daemon.prototype.__destroy__ = Promise.coroutine(function* destroy() {
  yield this.__stop__();
  yield rimraf(this.root);
});
...
//
Daemon.prototype.execute = Promise.coroutine(function* execute(queries, args) {
