...
var view = null;
var SELECTOR = '#search-reindex-task';
var ignore_runid = 0;
var last_runid = 0;
...
N.wire.on('navigate.done:admin.search.index', function search_reindex_widget_setup() {
  ...
  var current = N.runtime.page_data.search_reindex_task.current || 0;
  var total   = N.runtime.page_data.search_reindex_task.total || 1;
...
N.wire.once('navigate.done:admin.search.index', function search_reindex_widget_setup_handlers() {
  ...
  N.wire.on(module.apiPath + '.stop', function reindex_stop() {
    ...
    N.io.rpc('admin.search.index.reindex.stop')
      .then(function () {
        // reset progress bar to zero,
        // and ignore all updates on the last task
        ignore_runid = Math.max(prev_runid, ignore_runid);
